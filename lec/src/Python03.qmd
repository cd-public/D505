...
[print(v, var_cor(wine, v)) for v in vs]
```
...
g = sns.FacetGrid(filtered_wine, col="variety", hue="variety")
...
```{python}
X,y  = filtered_wine[['points']], filtered_wine['price']
filtered_wine['residual'] = y - LinearRegression().fit(X, y).predict(X)
sns.scatterplot(x='points', y='residual', data=filtered_wine, alpha=0.3)
```
...
def resid_plot(df, x, y):
  X,y  = df[[x]], filtered_wine[y]
  filtered_wine['residual'] = y - LinearRegression().fit(X, y).predict(X)
  return sns.scatterplot(x='points', y='residual', data=filtered_wine, alpha=0.3)
```
...
```{python}
resid_plot(filtered_wine, "points", "lprice")
```
...
def coef(df, x, y):
  X,y  = df[[x]], filtered_wine[y]
  return (LinearRegression().fit(X,y))_coef[0]
```
